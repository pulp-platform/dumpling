...
pins: Mapping[str, PinDecl] = {
    "chip_reset": {"name": "RSTN", "default": "1", "type": "input"},
    "trst": {"name": "JTAG_TRSTN", "default": "1", "type": "input"},
...
CLU_WAKEUP_REG: BitArray = BitArray(0x02040009) # CLINT reg to wake up cluster 4 core 0
CLU_RESET_REG: BitArray = BitArray(0x30001030) # Reset reg of cluster 4 (ITA)
CLU_CKGATE_REG: BitArray = BitArray(0x30001044)# Clk gate reg of cluster 4
...
def resume_core(vector_writer: HP93000VectorWriter, wait_cycles):
    ...
    with vector_writer as writer:
        ...
        vectors += riscv_debug_tap.resume_harts_no_loop(
            HOST_CORE_ID, "Resuming core", wait_cycles=wait_cycles
        )
...
def halt_core_verify_pc(
    ...
    E.g.::
    dumpling chimera -o halt_core halt_core_verify_pc --pc 0x1c008080 --resume
    ...
    with vector_writer as writer:
        ...
        vectors += riscv_debug_tap.halt_hart_no_loop(
            HOST_CORE_ID, wait_cycles=wait_cycles
        )
        if pc:
            ...
            if resume:
                vectors += riscv_debug_tap.resume_harts_no_loop(
                    HOST_CORE_ID, comment="Resuming the core", wait_cycles=wait_cycles
                )
        writer.write_vectors(vectors)
...
def verify_idcode(vector_writer):
    ...
    In Chimera, the value should match "0x1c5e5db3". After the idcode read-out, the debug unit TAP remains selected.
    """
    with vector_writer as writer:
...
def execute_elf(
    ...
    with writer as vector_writer:
        ...
        vectors = riscv_debug_tap.init_dmi()
        vectors += riscv_debug_tap.set_dmactive(True)
        vectors += riscv_debug_tap.halt_hart_no_loop(HOST_CORE_ID, wait_cycles=100)
        ...
        if verify:
            vectors = riscv_debug_tap.verify_elf(
                elf,
        ...
        vectors += riscv_debug_tap.resume_harts_no_loop(HOST_CORE_ID, wait_cycles=100)
        vector_writer.write_vectors(vectors, compress=compress)
