...
from dumpling.Common.VectorBuilder import PinDecl, VectorBuilder, NormalVector
...
@click.group()
@click.option(
    "--port-name", "-p", type=str, default="mini_jtag_and_reset_port", show_default=True
)
...
@click.pass_context
def appacapra(ctx, port_name, wtb_name, device_cycle_name, output):
    """Generate stimuli for the TSMC65 AppaCapra chip."""
...
def execute_elf(
    ...
    with writer as vector_writer:
        ...
        if verify:
             vectors = riscv_debug_tap.verify_elf(elf, wait_cycles=eoc_wait_cycles, comment="Verify the content of L2 to match the binary.")
             vector_writer.write_vectors(vectors, compress=compress)
...
def verify_mem(
    ...
    with vector_writer as writer:
        vector_builder.init()
        vectors = riscv_debug_tap.init_dmi()
        vectors += riscv_debug_tap.set_sbcs(sbreadonaddr=True)
        for address, value, comment in data:
            if loop:
                vectors += riscv_debug_tap.readMem(
                    addr=address, expected_data=value, comment=comment
