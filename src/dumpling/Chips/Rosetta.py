...
@click.option(
    "--wtb-name", "-w", type=str, default="multiport_ext_clk_wvtbl", show_default=True
)
@click.option(
    "--output",
    "-o",
...
@click.option(
    "--blade/--no-blade",
    default=True,
    help="Enables/Disables the BLADE SRAM macros in Rosetta",
    show_default=True,
)
@click.option(
    "--edram/--no-edram",
    default=True,
    help="Enables/Disables the eDRAM macros in Rosetta",
    show_default=True,
)
@click.option(
    "--hd-mem-backend",
    type=click.Choice(["edram", "scm"]),
    default="scm",
    show_default=True,
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Bypass the FLL for the SoC clock and use the external SoC clock instead.",
)
@click.option(
    "--bypass-per-fll",
    is_flag=True,
    default=False,
    help="Bypass the FLL for the Peripheral clock and use the external clock instead.",
)
@pass_VectorWriter
def write_soc_config(
    ...
    with vector_writer as writer:
        ...
        vectors += pulp_tap.set_config_reg(
            BitArray(8),  # type: ignore until https://github.com/scott-griffiths/bitstring/issues/276 is closed
            soc_fll_bypass_en=bypass_soc_fll,
            per_fll_bypass_en=bypass_per_fll,
            blade_disable=not blade,
            edram_disable=not edram,
...
def verify_soc_config(
    ...
    with vector_writer as writer:
        vectors = pulp_tap.init_pulp_tap()
        vectors += pulp_tap.verify_config_reg(
            BitArray(8),  # type: ignore until https://github.com/scott-griffiths/bitstring/issues/276 is closed
            soc_fll_bypass_en=bypass_soc_fll,
            per_fll_bypass_en=bypass_per_fll,
            blade_disable=not blade,
...
@click.option(
    ...
    default="scm",
    show_default=True,
    help="Switches between SCM and eDRAM as the memory backend for the HD-Computing "
    "Accelerator",
)
@click.option(
    "--bypass-soc-fll",
...
def write_mem(
    ...
    data = []
    pattern = re.compile(
        r"(?P<address>0x[0-9a-fA-F]{8})=(?P<value>0x[0-9a-fA-F]{0,8})(?:#(?P<comment>.*))?"
    )
...
@click.option(
    ...
    default=10,
    show_default=True,
    help="The number of cycles to wait for the read operation to complete.",
)
@pass_VectorWriter
def verify_mem(
    vector_writer: HP93000VectorWriter,
