...
class RISCVDebugTap(JTAGTap):
    ...
    def set_dmi(self, dm_op: DMIOp, dmi_addr: DMRegAddress, new_dm_data: str, expected_dm_status=None, expected_dm_data=None, comment=""):
    ...
    def wait_command(self, retries=1, comment=""):
    ...
    def set_command_no_loop(self, cmd: DMAbstractCmd, comment="", wait_cycles=10):
    ...
    def set_command(self, cmd: DMAbstractCmd, comment="", retries=1):
        comment += "/Issue abstract command register"
        # Write to command register
        vectors = self.write_debug_reg(DMRegAddress.COMMAND, cmd.to_bits().bin, verify_completion=False, comment=comment)
        # Wait for completion of the command
    ...
    def read_debug_reg(self, dmi_addr: DMRegAddress, expected_data: str, retries=1, comment=""):
    ...
    def write_debug_reg(self, dmi_addr: DMRegAddress, data: str, verify_completion=True, retries=1, comment=""):
    ...
    def write_reg_abstract_cmd_no_loop(self, reg: RISCVReg, data: BitArray, comment=""):
    ...
    def write_reg_abstract_cmd(self, reg: RISCVReg, data: BitArray, retries=1, comment=""):
        comment += "/Write {} to registers {}".format(data, reg)
    ...
    def read_reg_abstract_cmd(self, reg: RISCVReg, expected_data, retries=1, comment=""):
    ...
    def read_reg_abstract_cmd_no_loop(self, reg: RISCVReg, expected_data, wait_cycles=10, comment=""):
        comment += "Verify register {} equals {}".format(reg, expected_data)
    ...
    def halt_hart(self, hartsel: BitArray, comment="", retries=1):
    ...
    def halt_hart_no_loop(self, hartsel: BitArray, comment="", wait_cycles=10):
        comment += "/Halting hart 0x{}".format(hartsel.hex)
        new_dm_value = BitArray(32)
        new_dm_value[31] = 1  # haltreq
        new_dm_value[16:26] = hartsel[0:10]  # hartsello
    ...
    def resume_harts(self, hartsel: BitArray, comment="", retries=1):
    ...
    def resume_harts_no_loop(self, hartsel: BitArray, comment="", wait_cycles=10):
        comment += "/Resume hart {}".format(hartsel)
    ...
    def writeMem(self, addr: BitArray, data: BitArray, verify_completion=True, retries=1, comment=""):
    ...
    def readMem(self, addr: BitArray, expected_data: BitArray, retries=1, comment=""):
